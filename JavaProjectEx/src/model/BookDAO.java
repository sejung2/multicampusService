...
public class BookDAO implements IBookDAO{
    ...
    public Vector<BookDTO> getAllBook() throws Exception {
        ...
        try {
            ...
            pstmt = con.prepareStatement("select * from book2 order by bookNo");
    ...
    public boolean insert(BookDTO dto) throws Exception {
        ...
        try {
            con = DBConnect.getConnection();
            pstmt = con.prepareStatement("insert into book2 values(?, ?, ?, ?, ?, ?, ?)");
            ...
            throw e; // 호출한 쪽으로 예외처리하도록 넘김
        } finally {
            DBConnect.close(con, pstmt);
        }
        return true; // insert 된 행이 있는 경우
    }
    ...
    public boolean update(BookDTO dto) throws Exception {
        ...
        try {
            ...
            String sql = "update book2 set bookName=?, bookAuthor=?, bookPrice=?,"
                    + "bookDate=?, bookStock=?, where bookNo=?";
