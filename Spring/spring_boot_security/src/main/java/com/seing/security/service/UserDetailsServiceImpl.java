package com.seing.security.service;

import com.seing.security.domain.AppUser;
import com.seing.security.domain.AppUserRepository;
...
public class UserDetailsServiceImpl implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		if(user.isPresent()) {
			...
			builder = org.springframework.security.core.userdetails.User.withUsername(username); //User.Builer 객체 인스턴스 반환 == User.Builder().username()
		...
		return builder.build(); //스프링 시큐리티 인증로직에서 사용할 객체 반환(userDetails)
	}
